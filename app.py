...
# 5. รายชื่อบุคลากร (ตามข้อมูลที่คุณระบุมา)
list_supervisors = [
...
# ================= PAGE 2: APPLY REQUEST PERMIT =================
    ...
    with st.form("premium_solmax_form_v12"):
        ...
        # [ปรับปรุงใหม่]: เพิ่มช่องกรอกเวลาปฏิบัติงานจริงตามหน้า 1 และหน้า 2 สำหรับฟอร์ม Hot Work
        ...
        # ==============================================================================
        # [ปรับปรุงใหม่]: ส่วนตรวจสอบและประเมินค่าตัวเลขสำหรับ FM-HSE-001 (หน้า 1 & หน้า 2)
        # ==============================================================================
        hotwork_extra_data = {}
        if "FM-HSE-001" in permit_type:
            st.markdown("#### 🔬 ผลการตรวจวัดระดับก๊าซไวไฟ & แผนการเฝ้าระวังภัย")
            lel_val = st.number_input("ค่าก๊าซไวไฟวัดได้จริงก่อนเริ่มงาน (ค่า LEL %):", min_value=0.0, max_value=100.0, step=0.1, help="กรอกค่า LEL ที่วัดได้จากหน้างานจริง")
            ...
            st.markdown("#### 📝 ข้อมูลผู้เฝ้าระวังภัยและผู้พิจารณาขั้นสุดท้าย (อ้างอิงหน้า 2)")
            ...
            # ==============================================================================
            # [ปรับปรุงใหม่]: ส่วนเพิ่มข้อมูลฉุกเฉินและตารางปัจจัยโครงสร้าง (หน้า 3)
            # ==============================================================================
            st.markdown("#### 🚨 ระบบผู้ติดต่อกรณีฉุกเฉินหน้างาน & ข้อมูลอ้างอิง (อ้างอิงหน้า 3)")
            ...
            with st.expander("📊 คลิกเพื่อกางตารางอ้างอิงปัจจัยโครงสร้างและลักษณะพื้นที่เพื่อใช้คำนวณเวลา (หน้าที่ 3)"):
                st.markdown("""
                ...
                * **งานมุงหลังคาด้วยหัวเชื่อม:** เฝ้าระวังอย่างน้อย 2 ชม. และ ตรวจสอบอย่างน้อย 2 ชม. (หากใช้กล้องอินฟราเรด ลดเหลืออย่างละ 1 ชม.)
                * **งานบน/ในอุปกรณ์ที่ติดไฟและเลื่อนย้ายไม่ได้:** เฝ้าระวังอย่างน้อย 1 ชม. และ ตรวจสอบไฟอย่างน้อย 3 ชม. ทั้งภายในและบริเวณรอบๆ
                """)
                
            # รวมกลุ่มตัวแปรสำหรับเอาไปเซฟลงฐานข้อมูลร่วม
            hotwork_extra_data = {
                "lel_value": lel_val,
    ...
    if not active_permits:
        ...
        for idx, permit in enumerate(active_permits):
            ...
            with st.expander(f"📄 ใบงาน เลขที่: {permit['id']} | ประเภท: {permit['type'].split(':')[0]} | Status: {permit['status']}", expanded=True):
                ...
                # หากเป็นงาน Hot work แสดงค่าพารามิเตอร์การวัดค่าแก๊สและแผนเวลาให้ผู้อนุมัติเห็นประกอบการตัดสินใจเซ็น
                if "FM-HSE-001" in permit["type"] and "hotwork_details" in permit and permit["hotwork_details"]:
                    hw = permit["hotwork_details"]
                    st.info(f"📊 **ข้อมูลความปลอดภัย Hot Work**: ค่าก๊าซไวไฟ LEL: {hw['lel_value']}% | ผู้เฝ้าระวังภัย: {hw['after_work_watch_name']} (เสร็จสิ้น {hw['after_work_watch_end_time']}) | ผู้ตรวจไฟ: {hw['fire_inspector_name']}")
